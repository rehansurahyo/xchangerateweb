    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-[10px] font-black text-[#9FB0C7] uppercase tracking-[0.3em]">API Sessions</h2>
                    <p className="text-[9px] font-bold text-[#9FB0C7]/40 uppercase tracking-widest mt-0.5">
                        {sessions.length} configured · {sessions.filter(s => s.status === 'RUNNING').length} active
                    </p>
                </div>
                <button
                    onClick={() => setShowModal(true)}
                    className="flex items-center space-x-1.5 px-4 py-1.5 rounded-lg bg-[#1E293B] border border-white/10 text-white text-[9px] font-black uppercase tracking-widest hover:bg-[#1E293B]/80 transition-all"
                >
                    <Plus size={12} />
                    <span>Add</span>
                </button>
                        ...
                        <div className="p-0">
                            ...
                            <div className="flex flex-col md:flex-row items-center border-b border-white/5">
                                <div className="flex-1 p-3 flex items-center space-x-3 w-full">
                                    <div className={`w-1.5 h-1.5 rounded-full ${session.status === 'RUNNING' ? 'bg-[#22D3A6]' : 'bg-[#9FB0C7]/40'}`} />
                                    <div>
                                        <span className="text-xs font-bold text-white block">{session.name}</span>
                                    </div>
                                    <span className={`px-1.5 py-0.5 rounded text-[7px] font-black uppercase ${session.exchange === 'Binance' ? 'bg-yellow-400/10 text-yellow-400' :
                                ...
                                <div className="p-3 flex items-center justify-between w-full md:w-auto md:space-x-8">
                                    <span className={`text-[9px] font-black uppercase tracking-widest ${session.status === 'RUNNING' ? 'text-[#22D3A6]' : 'text-[#9FB0C7]/40'}`}>
                                        {session.status}
                                    </span>
                                </div>
                            </div>
                            ...
                            {/* Details Grid */}
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-white/[0.01]">
                                <div className="space-y-1">
                                    <p className="text-[9px] text-[#9FB0C7]/40 font-bold uppercase tracking-widest">API Key</p>
                                    <p className="text-[11px] font-mono text-[#9FB0C7]">abc123xyz789...</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-[9px] text-[#9FB0C7]/40 font-bold uppercase tracking-widest">Exchange</p>
                                    <p className={`text-[11px] font-bold uppercase ${session.exchange === 'Binance' ? 'text-yellow-400' :
                                        session.exchange === 'Bybit' ? 'text-orange-500' : 'text-blue-400'
                                        }`}>{session.exchange}</p>
                                </div>
                                <div className="space-y-1">
                                    <p className="text-[9px] text-[#9FB0C7]/40 font-bold uppercase tracking-widest">Created</p>
                                    <p className="text-[11px] font-mono text-[#9FB0C7]">2026-02-10 14:30</p>
                                </div>
                                <div className="space-y-1 text-right">
                                    <p className="text-[9px] text-[#9FB0C7]/40 font-bold uppercase tracking-widest">Total Trades</p>
                                    <p className="text-[11px] font-mono text-white">47</p>
                                </div>
                            </div>
                            ...
                            <div className="px-4 py-3 flex items-center justify-between border-t border-white/5">
                                <div className="flex items-center space-x-2">
                                    ...
                                    <span className="text-[9px] font-black text-white uppercase tracking-widest">Auto Restart</span>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <span className="text-[9px] font-black text-[#22D3A6] uppercase tracking-widest">ON</span>
                                    <div className="w-7 h-3.5 bg-primary rounded-full relative">
                            ...
                            <div className="flex border-t border-white/5 divide-x divide-white/5 bg-[#050A12]">
                                <button className={`flex-1 py-2 text-[9px] font-black uppercase tracking-widest hover:bg-white/5 transition-colors ${session.status === 'RUNNING' ? 'text-yellow-400' : 'text-[#22D3A6]'
                                    }`}>
                                    {session.status === 'RUNNING' ? 'STOP' : 'START'}
                                </button>
                                <button className="flex-1 py-2 text-[9px] font-black text-[#9FB0C7] uppercase tracking-widest hover:bg-white/5 hover:text-white transition-colors">
                                    IPS
                                </button>
                                <button className="flex-1 py-2 text-[9px] font-black text-[#9FB0C7]/60 uppercase tracking-widest hover:bg-white/5 hover:text-white transition-colors">
                                    CLOSE
                                </button>
                                <button className="flex-1 py-2 text-[9px] font-black text-red-500/60 uppercase tracking-widest hover:bg-red-500/10 hover:text-red-500 transition-colors">
                                    DEL
                                </button>
                            </div>
                        </div>
