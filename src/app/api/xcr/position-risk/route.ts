...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const upstreamRes = await fetch(`${UPSTREAM_API}/api/binance/position-risk`, {
            method: "POST",
            headers: {
        ...
        if (!upstreamRes.ok) {
            console.error(`[Proxy PositionRisk] Upstream Error ${upstreamRes.status}:`, data);
        }

        return NextResponse.json(data, { status: upstreamRes.status });
    } catch (error: any) {
        console.error("[Proxy PositionRisk] Exception:", error.message);
